...
class ParticleSystem {
  constructor(canvas, charSize) {
    ...
    this.columnStates = Array(this.columns).fill().map(() => ({
      active: false, 
      lastRestart: 0,
      initialSpeed: 2 + Math.random() * 3, // Keep initial speed for each column
      trailLength: 10 + Math.floor(Math.random() * (50 - 10)), // Keep initial trail length for each column
      particles: [] // New array to hold particles for each column
    })); // Enhanced to track initial settings
...
calculateRestartDelay(columnState) {
  const baseDelay = 50; // Base delay ensures there's always some pause
  const trailLengthDelay = columnState.trailLength * 20; // Adjusted multiplier for trail length
  const speedDelay = 500 / columnState.initialSpeed; // Delay based on speed

  // Calculate total delay
  const totalDelay = baseDelay + trailLengthDelay + speedDelay;
  return totalDelay;
}
...
const charSize = 18; // Adjust based on desired appearance
